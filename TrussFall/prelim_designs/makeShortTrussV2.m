...
C = [1, 0, 1, 1, zeros(1,15); %A
     1, 1, zeros(1,17); %B
     0, 1, 1, 0, 1, 1, zeros(1,13);             %C
     0, 0, 0, 1, 1, 0, 1, 1, zeros(1,11);    %D
...
Sx = zeros(joints, 3);
Sy = zeros(joints, 3);
Sx(2,1) = 1; Sy(2,2) = 1; Sy(joints,3) = 1; 

%Put a one where rxn forces are
%X and Y
 X = [7, 0, 16, 16, 25, 25, 34, 34, 42, 42, 55];
 Y = [0, 9, 9, 0, 9, 0, 9, 0, 9, 0, 9];
 
 %Make L
 %GET JOE TO GIVE ME THE MAGNITUDE OF LOAD
 
 %Minimum load is 4.9 N, Should be a column vector
 L = zeros(2 * joints, 1);
 L(joints + 6) = 4.9;
  
 %Uncomment when all issues are dealt with
 save('TrussShortDesignV2_JustinSadler_JosephGraham_PierreBoucher.mat','C','Sx','Sy','X','Y','L')
